...
<template>
  ...
  <table class="leaderboard-table">
     ...
     <tbody>
       <tr v-if="!personalLeaderBoard.length">
         <td :colspan="4" class="no-results">
           No games found for the selected filter.
         </td>
       </tr>
       <tr v-for="(game, rank) in personalLeaderBoard" :key="game.id">
         <td>{{ rank + 1 }}</td>
...
select {
 ...
 width: 100%; /* Ensure select elements take full width */
}
...
.leaderboard-table {
 ...
 table-layout: fixed; /* Prevents table from stretching on mobile */
}
...
@media (max-width: 480px) {
 ...
 .no-results {
   font-size: 14px; /* Adjust font size for no-results */
 }
}
</style>